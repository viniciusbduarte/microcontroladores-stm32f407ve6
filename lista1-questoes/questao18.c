...
int main(void)
{
  ...
  int linhas[4]  = {PIN_3, PIN_2, PIN_1, PIN_0};
  int colunas[4] = {PIN_4, PIN_5, PIN_6, PIN_7};
  char buffer[50];

  for(int i = 0; i < 4; i++){
      GPIO_Pin_Mode(GPIOD, linhas[i], OUTPUT);
      GPIO_Write_Pin(GPIOD, linhas[i], LOW);
  }

  for(int i = 0; i < 4; i++){
      GPIO_Pin_Mode(GPIOD, colunas[i], INPUT);
      GPIO_Resistor_Enable(GPIOD, colunas[i], PULL_DOWN);
  }



  uint8_t seg[8][8] = {{ 0b0111111, 0b0000110, 0b1011011, 0b1001111 },
  ...
  GPIOA->ODR = (GPIOA->ODR & ~(0x7F)) | seg[0][0];
  ...
  while (1)
  {
	  ...
	  for(int linha = 0; linha < 4; linha++)
	      {
	          GPIO_Write_Pin(GPIOD, linhas[linha], HIGH);
	          Delay_us(5);
	          for(int coluna = 0; coluna < 4; coluna++)
	          {
	              if(GPIO_Read_Pin(GPIOD, colunas[coluna]))
	              {
	                  sprintf(buffer, "Tecla L%d C%d pressionada\r\n", linha, coluna);
	            	  GPIOA->ODR = (GPIOA->ODR & ~(0x7F)) | seg[linha][coluna];
	                  HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
	                  Delay_ms(200);
	              }
	          }

	          GPIO_Write_Pin(GPIOD, linhas[linha], LOW);
	      }
