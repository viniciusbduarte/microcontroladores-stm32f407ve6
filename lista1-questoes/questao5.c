...
int main(void)
{
  ...
  static uint16_t estado_anterior = 1;
  static uint16_t estado_anterior1 = 1;
  ...
  while (1)
  {
	    uint16_t estado_botao = GPIO_Read_Pin(GPIOE, PIN_4);

	    uint16_t estado_botao1 = GPIO_Read_Pin(GPIOE, PIN_3);
	    ...
	    if (estado_botao == 0 && estado_anterior == 1) {
	    ...
	    if (estado_botao1 == 0 && estado_anterior1 == 1) {


	        GPIO_Write_Pin(GPIOA, PIN_7, LOW);

	    }
	    estado_anterior1 = estado_botao1;
