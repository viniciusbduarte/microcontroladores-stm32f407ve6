...
int main(void)
{
  ...
  while (1)
  {
      for (int rodada = 0; rodada < 10; rodada++) {
          ...
          for (int piscada = 0; piscada <= rodada; piscada++) {
              int aleatorio = rand() % 4; // número entre 0 e 3
              sequencia[rodada][piscada] = aleatorio;

              GPIO_Write_Pin(GPIOA, leds[aleatorio], HIGH);
              Delay_ms(300);
              GPIO_Write_Pin(GPIOA, leds[aleatorio], LOW);
              Delay_ms(300);
          }
          ...
          while (flag && n <= rodada) {
              for (int i = 0; i < 4; i++) {
                  if (GPIO_Read_Pin(GPIOE, botoes[i]) == 0) { // botão pressionado
                      GPIO_Write_Pin(GPIOA, leds[i], HIGH);
                      Delay_ms(200);
                      GPIO_Write_Pin(GPIOA, leds[i], LOW);

                      if (sequencia[rodada][n] == i) {
                          n++; // acerto
                          if (n > rodada) flag = 0; // passou rodada
                      } else {
                          // Errou — pisca todos LEDs
                          for (int j = 0; j < 3; j++) {
